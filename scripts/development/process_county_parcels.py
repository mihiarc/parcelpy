...
class CountyParcelProcessor:
    ...
    def _calculate_optimal_chunk_size(self, county_parcels: gpd.GeoDataFrame) -> int:
        ...
        # Calculate size based on different constraints
        
        # 1. Memory constraint (4GB target)
        memory_based_size = int(4e9 / memory_per_parcel)
        
        # 2. Time constraint (target 3-minute processing time)
        # Assume 0.1 seconds per parcel per year as baseline
        time_per_parcel = 0.1 * len(self.years)
        time_based_size = int(180 / time_per_parcel)  # 180 seconds = 3 minutes
        
        # 3. Network constraint (assume 1MB per 100 parcels per year)
        network_mb_per_parcel = 0.01 * len(self.years)
        network_based_size = int(50 / network_mb_per_parcel)  # 50MB target per chunk
        
        # 4. Payload size constraint (10MB limit)
        # Estimate ~1KB per parcel geometry on average
        payload_bytes_per_parcel = 1024  # 1KB per parcel geometry
        payload_based_size = int(9 * 1024 * 1024 / payload_bytes_per_parcel)  # 90% of 10MB limit
    ...
    def _process_large_parcel_timeseries(
        ...
        def process_year(year):
            ...
            # Return 0 if no result (ensures we always have a value)
    ...
    def _process_sub_resolution_timeseries(
        ...
        def process_year(year):
            ...
            results = area_image.reduceRegion(
             ...
             # Find category with maximum area, return 0 if no result
    ...
    def _monitor_tasks(
        ...
        """Monitor export tasks and track their completion status."""
    ...
    def process_county(
        ...
        # Simplify geometries to reduce payload size (1m tolerance)
        logger.info("Simplifying geometries to reduce payload size...")
        simplified_parcels = county_parcels.copy()
        simplified_parcels['geometry'] = simplified_parcels.geometry.simplify(1.0)
        
        # Convert to Earth Engine FeatureCollection
        county_fc = geemap.geopandas_to_ee(simplified_parcels)
        
        # Use a more conservative chunk size (100 parcels)
        chunk_size = 100
